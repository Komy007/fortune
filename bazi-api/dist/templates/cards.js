...
exports.InterpretationTemplates = void 0;
/**
 * 전문가급 명리학 해석 템플릿
 */
class InterpretationTemplates {
     ...
     */
    generateExpertInterpretation(baziResult) {
     ...
     * 재물운 해석
     */
    generateWealthAnswer(baziResult) {
        const dayElement = baziResult.dayElement;
        const elementCounts = baziResult.elementCounts;
        ...
        const wealthStrength = elementCounts[wealthElement] || 0;
        let oneLiner = '';
        let why = '';
        let todo = [];
        if (wealthStrength >= 3) {
     ...
     * 직업운 해석
     */
    generateCareerAnswer(baziResult) {
        const dayElement = baziResult.dayElement;
        const elementCounts = baziResult.elementCounts;
        ...
        const careerElement = careerElements[dayElement];
        const careerStrength = elementCounts[careerElement] || 0;
        let oneLiner = '';
        let why = '';
        let todo = [];
        if (careerStrength >= 2) {
     ...
     * 투자 타이밍 해석
     */
    generateInvestmentAnswer(baziResult) {
        const dayElement = baziResult.dayElement;
     ...
     * 인연운 해석
     */
    generateRelationshipAnswer(baziResult) {
        const dayElement = baziResult.dayElement;
        const elementCounts = baziResult.elementCounts;
        ...
        const relationshipElement = relationshipElements[dayElement];
        const relationshipStrength = elementCounts[relationshipElement] || 0;
        let oneLiner = '';
        let why = '';
        let todo = [];
     ...
     * 건강운 해석
     */
    generateHealthAnswer(baziResult) {
        const dayElement = baziResult.dayElement;
        const elementCounts = baziResult.elementCounts;
        let oneLiner = '';
        let why = '';
        ...
        const healthOrgans = healthElements[dayElement];
     ...
     * 이사/이동운 해석
     */
    generateMovingAnswer(baziResult) {
        const dayElement = baziResult.dayElement;
        ...
        const currentSeasonElement = seasonalElements[currentMonth];
     ...
     * 올해/이번달 운세 해석
     */
    generateYearlyAnswer(baziResult) {
        const currentYear = new Date().getFullYear();
        const yearPillar = baziResult.pillars.year;
        const dayElement = baziResult.dayElement;
        let oneLiner = '';
     ...
     * 가장 강한 오행 찾기
     */
    getStrongestElement(elementCounts) {
        return Object.entries(elementCounts).reduce((a, b) => elementCounts[a[0]] > elementCounts[b[0]] ? a : b)[0];
    }
    /**
     * 가장 약한 오행 찾기
     */
    getWeakestElement(elementCounts) {
        return Object.entries(elementCounts).reduce((a, b) => elementCounts[a[0]] < elementCounts[b[0]] ? a : b)[0];
    }
    /**
     * 오행 균형 점수 계산
     */
    calculateBalanceScore(elementCounts) {
     ...
     * 오행 상생 관계 확인
     */
    isGenerating(producer, produced) {
        const generationMap = {
            '목': '화', '화': '토', '토': '금', '금': '수', '수': '목'
        };
     ...
     * 개인화된 추천사항 생성
     */
    generateRecommendations(baziResult) {
        const dayElement = baziResult.dayElement;
        const elementCounts = baziResult.elementCounts;
