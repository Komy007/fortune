...
exports.SajuAdapter = void 0;
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
class SajuAdapter {
    constructor(dbPath) {
        this.db = new better_sqlite3_1.default(dbPath);
    }
    async getSajuProfile(userId) {
        try {
            // 사용자 기본 정보 조회
            const userQuery = `
        SELECT 
        ...
        WHERE u.id = ?
      `;
            const user = this.db.prepare(userQuery).get(userId);
            if (!user) {
                return null;
            }
            // 사주 정보가 없는 경우 기본 오행 계산
            if (!user.year_element) {
                const calculatedSaju = this.calculateBasicSaju(user.birth_year, user.birth_month, user.birth_day, user.birth_hour);
                return {
    ...
    calculateBasicSaju(year, month, day, hour) {
...
exports.SajuAdapter = SajuAdapter;
