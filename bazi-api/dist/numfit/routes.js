    ...
    try {
        ...
        for (let date = new Date(start); date <= end; date.setDate(date.getDate() + 1)) {
            ...
            if (preferredElements && preferredElements.length > 0) {
                shouldInclude = preferredElements.some(element => analysis.elements.includes(element));
            }
            if (avoidElements && avoidElements.length > 0) {
                shouldInclude = !avoidElements.some(element => analysis.elements.includes(element));
            }
            if (shouldInclude) {
                selectedDates.push({
                    date: dateStr,
...
router.post('/batch-analyze', async (req, res) => {
    try {
        ...
        // 모든 숫자 분석
        const analyzer = new NumberFitAnalyzer(sajuProfile);
        const analyses = numbers.map(num => analyzer.analyzeNumber(num.number, num.type));
        // 요약 정보 생성
