...
exports.NumberFitAnalyzer = void 0;
const mapping_1 = require("./mapping");
class NumberFitAnalyzer {
    constructor(sajuProfile) {
        this.sajuProfile = sajuProfile;
    }
    analyzeNumber(number, type) {
    ...
    extractElements(number) {
        const digits = number.replace(/\D/g, '').split('').map(d => parseInt(d));
        const elements = [];
        let previousElement;
        for (const digit of digits) {
            const element = (0, mapping_1.getElementFromDigit)(digit, previousElement);
            elements.push(element);
            previousElement = element;
        }
        return elements;
    }
    calculateScore(elements, type) {
        let baseScore = 50;
        let weightedSum = 0;
        let totalWeight = 0;
        // 사주 오행과의 조화도 계산
        ...
        // 각 숫자 위치별 가중치 적용
        const weights = mapping_1.POSITION_WEIGHTS[type] || mapping_1.POSITION_WEIGHTS.personal;
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            for (const sajuElement of sajuElements) {
                const relationship = (0, mapping_1.analyzeElementRelationship)(element, sajuElement);
                elementScore += relationship.strength * 10;
            }
            // 중요 오행과의 관계 분석
            for (const importantElement of importantElements) {
                if (importantElement) {
                    const relationship = (0, mapping_1.analyzeElementRelationship)(element, importantElement);
                    elementScore += relationship.strength * 15;
                }
            }
            // 계절적 강도 고려
            const seasonalStrength = (0, mapping_1.getSeasonalStrength)(element, this.sajuProfile.month);
    ...
    getPositionWeight(index, totalLength, weights) {
        if (totalLength <= 3)
            return 1.0;
        const firstThird = Math.floor(totalLength / 3);
        const lastThird = totalLength - Math.floor(totalLength / 3);
        if (index < firstThird)
            return weights.first || 0.4;
        if (index >= lastThird)
            return weights.last || 0.3;
        return weights.middle || 0.3;
    }
    calculatePatternBonus(elements) {
        let bonus = 0;
        // 연속된 같은 오행 보너스
        ...
        // 상생 관계 보너스
        for (let i = 0; i < elements.length - 1; i++) {
            const relationship = (0, mapping_1.analyzeElementRelationship)(elements[i], elements[i + 1]);
    ...
    calculateDigitalRootBonus(number) {
        const digitalRoot = (0, mapping_1.calculateDigitalRoot)(number);
        const luckyNumbers = (0, mapping_1.getLuckyNumbersByElement)(this.sajuProfile.yongshin || '토');
        const avoidNumbers = (0, mapping_1.getAvoidNumbersByElement)(this.sajuProfile.yongshin || '토');
        if (luckyNumbers.includes(digitalRoot))
            return 5;
        if (avoidNumbers.includes(digitalRoot))
            return -3;
        return 0;
    }
    calculateCompatibility(elements) {
        ...
        for (const element of elements) {
            if (yongshin) {
                const relationship = (0, mapping_1.analyzeElementRelationship)(element, yongshin);
                yongshinScore += relationship.strength * 10;
            }
            if (heeshin) {
                const relationship = (0, mapping_1.analyzeElementRelationship)(element, heeshin);
    ...
    generateAnalysis(elements, score, type) {
        const positive = [];
        const negative = [];
        const suggestions = [];
        // 긍정적 분석
        ...
        if (this.sajuProfile.yongshin) {
            const luckyNumbers = (0, mapping_1.getLuckyNumbersByElement)(this.sajuProfile.yongshin);
    ...
    calculateFortune(elements, score) {
...
exports.NumberFitAnalyzer = NumberFitAnalyzer;
