 ...
 * 1단계: 기본 회원가입 (계정 생성)
 */
export async function basicSignUp(input: BasicSignUpInput): Promise<{ userId: string; token: string }> {
  // 이메일 중복 검사
 ...
 * 2단계: 출생 정보 저장
 */
export async function saveBirthData(userId: string, input: BirthDataInput): Promise<void> {
  // 기존 출생 정보가 있는지 확인
 ...
 * 3단계: 사용자 설정 저장
 */
export async function saveUserPrefs(userId: string, input: UserPrefsInput): Promise<void> {
 ...
 * 완전한 회원가입 (모든 단계 한번에)
 */
export async function completeSignUp(input: CompleteSignUpInput): Promise<AuthResponse> {
  // 이메일 중복 검사
  const existingUser = await prisma.user.findUnique({
    where: { email: input.email }
  });
 ...
 * 로그인 서비스 (확장된 정보 포함)
 */
export async function loginExtended(input: LoginInput): Promise<AuthResponse> {
  // 사용자 조회
 ...
 */
export async function getUserByIdExtended(userId: string) {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    include: { 
      prefs: true,
