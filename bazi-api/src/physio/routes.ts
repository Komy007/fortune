...
import { PhysioInputSchema, PhysioResponseSchema } from './schemas';
import { analyzePhysiognomy, getCacheKey } from './service';
...
router.post('/analyze', async (req, res) => {
  try {
    ...
    // 입력 데이터 검증
    const validationResult = PhysioInputSchema.safeParse(req.body);
    ...
    const result = await analyzePhysiognomy(input);
    
    // 결과 검증
    const resultValidation = PhysioResponseSchema.safeParse({
      success: true,
      result
    });
