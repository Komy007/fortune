...
import { analyzeForehead, analyzeEyes, analyzeNose, analyzeMouth, analyzeJaw, analyzeAsymmetry, PHYSIO_TEMPLATES, GOAL_COACHING_TEMPLATES } from './rules';
import { generateCacheKey } from './schemas';

// 이미지 품질 평가 함수 (임시 구현)
export function assessImageQuality(imageBase64: string): QualityMetrics {
  ...
  const issues: string[] = [];
  if (frontalScore < 0.7) issues.push('정면 사진 품질이 낮습니다');
  if (profileScore < 0.7) issues.push('측면 사진 품질이 낮습니다');
...
// 얼굴 랜드마크 추출 함수 (임시 구현)
export function extractFaceRatios(frontalImage: string, profileImage: string): FaceRatios {
...
// 관상학 분석 수행
export function performPhysioAnalysis(ratios: FaceRatios) {
  ...
  const asymmetry = analyzeAsymmetry(ratios);
...
export function generateLLMPrompt(
  analysis: ReturnType<typeof performPhysioAnalysis>,
  goal: string,
  lang: string
): string {
  const templates = PHYSIO_TEMPLATES[lang as keyof typeof PHYSIO_TEMPLATES] || PHYSIO_TEMPLATES.ko;
  const goalTemplate = GOAL_COACHING_TEMPLATES[goal as keyof typeof GOAL_COACHING_TEMPLATES] || GOAL_COACHING_TEMPLATES.career;
  const goalLang = goalTemplate[lang as keyof typeof goalTemplate] || goalTemplate.ko;
...
// 메인 분석 함수
export async function analyzePhysiognomy(input: PhysioInput): Promise<PhysioAnalysis> {
  try {
    // 1. 이미지 품질 평가
...
// 요약 생성 함수
function generateSummary(analysis: ReturnType<typeof performPhysioAnalysis>, lang: string): string {
  const templates = PHYSIO_TEMPLATES[lang as keyof typeof PHYSIO_TEMPLATES] || PHYSIO_TEMPLATES.ko;
  
  const keyFeatures = [];
  if (analysis.forehead.upper.balanced) keyFeatures.push('균형잡힌 이마');
  if (analysis.eyes.spacing.type === 'balanced') keyFeatures.push('균형잡힌 눈 간격');
  if (analysis.nose.straightness.type === 'straight') keyFeatures.push('직선적 코');
  if (analysis.mouth.philtrum.type === 'balanced') keyFeatures.push('균형잡힌 입');
  if (analysis.jaw.chin.type === 'moderate') keyFeatures.push('적당한 턱');
...
// 캐시 키 생성
export function getCacheKey(input: PhysioInput): string {
