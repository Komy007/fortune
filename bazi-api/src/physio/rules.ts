...
import { FaceRatios, PHYSIO_RULES, PhysioTemplates } from './types';

// 관상학 규칙 매핑 함수들
export function analyzeForehead(ratios: FaceRatios) {
  ...
  const results = {
    upper: {
      balanced: Math.abs(upperRatio - PHYSIO_RULES.FOREHEAD_RATIOS.IDEAL_UPPER) < 0.05,
      ratio: upperRatio,
      coaching: ''
    },
    middle: {
      balanced: Math.abs(middleRatio - PHYSIO_RULES.FOREHEAD_RATIOS.IDEAL_MIDDLE) < 0.05,
      ratio: middleRatio,
      coaching: ''
    },
    lower: {
      balanced: Math.abs(lowerRatio - PHYSIO_RULES.FOREHEAD_RATIOS.IDEAL_LOWER) < 0.05,
      ratio: lowerRatio,
      coaching: ''
    }
  };
...
export function analyzeEyes(ratios: FaceRatios) {
  const { eyeSpacing, eyeCornerAngle } = ratios;
  
  const results = {
    spacing: {
      type: eyeSpacing < PHYSIO_RULES.EYE_SPACING.NARROW ? 'narrow' : 
            eyeSpacing > PHYSIO_RULES.EYE_SPACING.WIDE ? 'wide' : 'balanced',
      ratio: eyeSpacing,
      coaching: ''
    },
    angle: {
...
export function analyzeNose(ratios: FaceRatios) {
  const { nasalStraightness, nasalProjection } = ratios;
  
  const results = {
    straightness: {
      type: nasalStraightness > PHYSIO_RULES.NASAL_STRAIGHTNESS.STRAIGHT ? 'straight' :
                nasalStraightness < PHYSIO_RULES.NASAL_STRAIGHTNESS.CURVED ? 'curved' : 'moderate',
      score: nasalStraightness,
      coaching: ''
    },
    projection: {
      type: nasalProjection > PHYSIO_RULES.CHIN_PROJECTION.STRONG ? 'strong' :
              nasalProjection < PHYSIO_RULES.CHIN_PROJECTION.WEAK ? 'weak' : 'moderate',
...
export function analyzeMouth(ratios: FaceRatios) {
  ...
  const results = {
    ...
    lipCorner: {
      ...
      type: lipCornerSlope > PHYSIO_RULES.LIP_CORNER_SLOPE.UPWARD ? 'upward' :
            lipCornerSlope < PHYSIO_RULES.LIP_CORNER_SLOPE.DOWNWARD ? 'downward' : 'horizontal',
      coaching: ''
    }
  };
...
export function analyzeJaw(ratios: FaceRatios) {
  const { chinProjection, jawAngle, jawWidthToCheek } = ratios;
  
  const results = {
    chin: {
      projection: chinProjection,
      type: chinProjection > PHYSIO_RULES.CHIN_PROJECTION.STRONG ? 'strong' :
            chinProjection < PHYSIO_RULES.CHIN_PROJECTION.WEAK ? 'weak' : 'moderate',
      coaching: ''
    },
    angle: {
      degree: jawAngle,
...
export function analyzeAsymmetry(ratios: FaceRatios) {
...
// 언어별 템플릿
export const PHYSIO_TEMPLATES: PhysioTemplates = {
  ko: {
    title: '관상학 전문 분석 리포트',
...
// 목표별 코칭 템플릿
export const GOAL_COACHING_TEMPLATES = {
  career: {
    ko: {
      title: '경력 발전 코칭',
