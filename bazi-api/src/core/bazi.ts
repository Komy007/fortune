...
import { jieqiCalculator } from './jieqi';
import { GanzhiCalculator } from '../utils/ganzhi';

export class BaziEngine {
  private ganzhi: GanzhiCalculator;

  constructor() {
    this.ganzhi = new GanzhiCalculator();
  }
   ...
   * 출생 시간 파싱
   */
  private parseBirthDateTime(birthData: BirthData): DateTime {
   ...
   * 경계 감도 검사
   */
  private checkBoundarySensitivity(birthDateTime: DateTime, unknownTime?: boolean): BoundarySensitivity {
    const jieqiCheck = jieqiCalculator.checkBoundarySensitivity(birthDateTime);
    ...
    let confidence = Math.min(
      jieqiCheck.confidence,
      hasHourBoundary ? 0.8 : 1.0,
      unknownTime ? 0.6 : 1.0
    );
   ...
   * 사주 기둥 계산
   */
  private calculatePillars(birthDateTime: DateTime): Pillars {
    ...
    // 월주 계산 (중기 기준)
    const monthInfo = jieqiCalculator.getMonthPillarByJieQi(birthDateTime);
   ...
   * 대안 기둥 생성
   */
  private generateAlternativePillars(birthDateTime: DateTime): Pillars[] {
    const alternatives: Pillars[] = [];
   ...
   * 중복 기둥 제거
   */
  private removeDuplicatePillars(pillars: Pillars[]): Pillars[] {
    const seen = new Set<string>();
    const unique: Pillars[] = [];
   ...
   * 오행 분석
   */
  private analyzeFiveElements(pillars: Pillars): FiveElements {
    ...
    [pillars.year.stem, pillars.month.stem, pillars.day.stem, pillars.hour.stem].forEach(stem => {
      const element = this.ganzhi.getStemElement(stem as HeavenlyStem);
      elements[element as keyof FiveElements]++;
    });
    
    // 지지 오행 (숨겨진 천간 포함)
    [pillars.year.branch, pillars.month.branch, pillars.day.branch, pillars.hour.branch].forEach(branch => {
      const element = this.ganzhi.getBranchElement(branch as EarthlyBranch);
      elements[element as keyof FiveElements]++;
      
      // 숨겨진 천간의 오행도 추가
      const hiddenStems = this.ganzhi.getHiddenElements(branch as EarthlyBranch);
      hiddenStems.forEach(stem => {
        const hiddenElement = this.ganzhi.getStemElement(stem as HeavenlyStem);
        elements[hiddenElement as keyof FiveElements]++;
      });
    });
    ...
    Object.keys(elements).forEach(key => {
      elements[key as keyof FiveElements] = Math.round((elements[key as keyof FiveElements] / total) * 100) / 100;
    });
   ...
   * 십신 강도 분석
   */
  private analyzeTenGods(pillars: Pillars): TenGodsStrength {
    const dayMaster = pillars.day.stem;
    const tenGods: TenGodsStrength = {
      wealth: 0,    // 재성
      officer: 0,   // 관성
      output: 0,    // 식상
      resource: 0,  // 인성
    ...
    allStems.forEach(stem => {
      const tenGod = this.ganzhi.getTenGod(dayMaster as HeavenlyStem, stem as HeavenlyStem);
      tenGods[tenGod as keyof TenGodsStrength]++;
    });
    ...
    Object.keys(tenGods).forEach(key => {
      tenGods[key as keyof TenGodsStrength] = Math.round((tenGods[key as keyof TenGodsStrength] / total) * 100) / 100;
    });
   ...
   * 용신 후보 선정
   */
  private selectUsefulGods(fiveElements: FiveElements, tenGods: TenGodsStrength): string[] {
   ...
   * 타임라인 생성
   */
  private generateTimeline(birthDateTime: DateTime, sexAtBirth: 'male' | 'female'): Timeline {
    ...
    for (let i = -1; i <= 1; i++) {
      ...
      const monthInfo = jieqiCalculator.getMonthPillarByJieQi(targetMonth);
  ...
  // 헬퍼 메서드들
  private getLuckPillarTheme(pillar: Pillar): string {
    const themes = {
      '갑': '성장과 확장',
      '을': '유연한 발전',
      '병': '열정과 변화',
    ...
    return themes[pillar.stem as keyof typeof themes] || '균형과 조화';
  }

  private getAnnualFocus(pillar: Pillar): string {
    const focuses = {
      '갑': '새로운 시작과 도전',
      '을': '협력과 성장',
      '병': '열정과 창의성',
    ...
    return focuses[pillar.stem as keyof typeof focuses] || '균형잡힌 발전';
  }

  private getMonthlyWindow(month: DateTime): string {
    const start = month.startOf('month');
    const end = month.endOf('month');
    return `${start.toFormat('yyyy-MM-dd')}~${end.toFormat('yyyy-MM-dd')}(중기 기준)`;
  }

  private getMonthlyActions(pillar: Pillar): string[] {
    return [
      '새로운 계획 수립',
      '기존 프로젝트 정리',
      '관계 개선 노력'
  ...
  // 카드별 생성 메서드들
  private generateWealthCard(fiveElements: FiveElements, tenGods: TenGodsStrength): AnswerCard {
  ...
  private generateCareerCard(fiveElements: FiveElements, tenGods: TenGodsStrength): AnswerCard {
  ...
  private generateRelationshipCard(fiveElements: FiveElements, tenGods: TenGodsStrength): AnswerCard {
  ...
  private generateHealthCard(fiveElements: FiveElements, usefulGods: string[]): AnswerCard {
  ...
  private generateTimingCard(fiveElements: FiveElements, tenGods: TenGodsStrength): AnswerCard {
