...
export class GanzhiCalculator {
  // 천간 정의
  private readonly HEAVENLY_STEMS: HeavenlyStem[] = ['갑', '을', '병', '정', '무', '기', '경', '신', '임', '계'];
  
  // 지지 정의
  private readonly EARTHLY_BRANCHES: EarthlyBranch[] = ['자', '축', '인', '묘', '진', '사', '오', '미', '신', '유', '술', '해'];
  
  // 천간 오행 매핑
  private readonly STEM_ELEMENTS: Record<HeavenlyStem, Element> = {
    '갑': '목', '을': '목',
    '병': '화', '정': '화',
    '무': '토', '기': '토',
    '경': '금', '신': '금',
  ...
  // 지지 오행 매핑
  private readonly BRANCH_ELEMENTS: Record<EarthlyBranch, Element> = {
    '자': '수', '축': '토', '인': '목', '묘': '목',
    '진': '토', '사': '화', '오': '화', '미': '토',
    '신': '금', '유': '금', '술': '토', '해': '수'
  };
  ...
  // 지지 숨겨진 천간
  private readonly HIDDEN_STEMS: Record<EarthlyBranch, HeavenlyStem[]> = {
    '자': ['계'],
    '축': ['기', '신', '계'],
    '인': ['병', '무'],
    '묘': ['을'],
   ...
   * 연주 계산
   */
  calculateYearPillar(year: number): { stem: HeavenlyStem; branch: EarthlyBranch; element: Element } {
   ...
   * 월주 계산
   */
  calculateMonthPillar(year: number, month: number): { stem: HeavenlyStem; branch: EarthlyBranch; element: Element } {
   ...
   * 일주 계산 (간단한 근사 계산)
   */
  calculateDayPillar(year: number, month: number, day: number): { stem: HeavenlyStem; branch: EarthlyBranch; element: Element } {
   ...
   * 시주 계산
   */
  calculateHourPillar(dayStem: HeavenlyStem, hour: number): { stem: HeavenlyStem; branch: EarthlyBranch; element: Element } {
  ...
  getTenGod(dayMaster: HeavenlyStem, targetStem: HeavenlyStem): string {
    ...
    // 십신 관계 정의
    const tenGodRelations: Record<Element, Record<Element, string>> = {
      '목': {
        '목': 'peer',      // 비겁
        '화': 'output',    // 식상
        '토': 'wealth',    // 재성
