...
export class TimeUtils {
   ...
   * 태양시 계산 (실제 태양의 위치 기반)
   */
  public calculateSolarTime(dateTime: DateTime, longitude: number): DateTime {
    // 경도에 따른 시간 보정 (1도 = 4분)
   ...
   * 절기 기준 월 계산
   */
  public getJieQiMonth(dateTime: DateTime): number {
    const month = dateTime.month;
    const day = dateTime.day;
    
    // 절기 기준 월 계산 (간략화)
    if (month === 1 && day < 5) return 11; // 소한 이전
    if (month === 2 && day < 4) return 0;  // 입춘 이전
    if (month === 3 && day < 5) return 1;  // 경칩 이전
    if (month === 4 && day < 4) return 2;  // 청명 이전
    if (month === 5 && day < 5) return 3;  // 입하 이전
    if (month === 6 && day < 5) return 4;  // 망종 이전
    if (month === 7 && day < 6) return 5;  // 소서 이전
    if (month === 8 && day < 7) return 6;  // 입추 이전
    if (month === 9 && day < 7) return 7;  // 백로 이전
    if (month === 10 && day < 8) return 8; // 한로 이전
    if (month === 11 && day < 7) return 9; // 입동 이전
    if (month === 12 && day < 7) return 10; // 대설 이전
   ...
   * 시간대별 지지 반환
   */
  public getHourBranch(hour: number): string {
   ...
   * 음력 날짜 변환 (간략화)
   */
  public convertToLunar(dateTime: DateTime): {
    year: number;
    month: number;
    day: number;
    isLeap: boolean;
   ...
   * 시간대별 운세 강도 계산
   */
  public calculateHourLuck(hour: number): {
    strength: number;
    description: string;
  } {
    // 시간대별 운세 강도 (간단한 예시)
    const hourLuckMap: Record<number, { strength: number; description: string }> = {
